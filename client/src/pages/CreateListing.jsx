...
const handleImageSubmit = async (e) => {
 ...
 const storeImage = (file) => {
  return new Promise((resolve, reject) => {
    ...
    xhr.open(
      "POST",
      "https://api.cloudinary.com/v1_1/dx3wgazyt/image/upload"
    );
...
const handleSubmit= async (e) => {
  ...
  try{
    if (formData.imageUrls.length < 1 )  return setError('You must upload atleast one image')
      if(+formData.regularPrice < +formData.discountPrice) return setError('DiscountPrice must be lower than regularPrice')
setLoading(true);
setError(false);
...
<div className='flex flex-col flex-1 gap-4'>
    <p className='font-semibold'>
        Images:
        <span className='font-normal text-gray-600 ml-2'> The first image will be the cover (max) </span>
    </p>
<div className='flex gap-4 '>
<input onChange ={(e)=> setFiles([...e.target.files])}  className='p-3 border border-gray-700 rounded-lg w-full' 
type="file" 
id='images' 
accept='image/*'
 multiple  />
<button type='button' onClick={handleImageSubmit} className='p-3 text-green-700 border
